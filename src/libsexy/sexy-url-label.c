...
/*
 * sexy_url_label_size_allocate() has been modified, because the
 * original function didn't update the height of the label.
 *
 * sexy_url_label_motion_notify_event() didn't set the active link
 * correctly. This issue has been fixed also.
 *
 * Added clipboard support for Microsoft Windows operating systems
 * in copy_link_activate_cb().
 *
 * sexy_url_label_finalize didn't destroy popup_menu & event_window.
 *
 *                           Sebastian Fedrau, January-Feburary 2011
 */
...
sexy_url_label_motion_notify_event(GtkWidget *widget, GdkEventMotion *event)
{
	...
	if (found)
	{
		// if (priv->active_link == NULL) This will not update the active link correctly. -- Sebastian Fedrau
		{
			GdkCursor *cursor;
...
sexy_url_label_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
	...
	gtk_widget_set_size_request(widget, -1, log.height);
